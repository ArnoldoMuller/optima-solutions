...
// Recebe notificações do Mercado Pago, atualiza o registro no Supabase
// (que já foi pré-salvo pelo create-preference.js),
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // ── 3. Atualiza inscrição existente (pre-salva pelo create-preference) ─
    //    Estratégia: PATCH pelo external_reference (chave confiável do MP)
    //    Se não existir ainda, faz INSERT como fallback.

    // Campos sempre atualizados (vêm do MP, são confiáveis)
    const updatePayload = {
      ...
      taxa_mp:          parseFloat(totalTaxas.toFixed(2)) || null
    };

    // Dados do participante: usa metadata se disponível, senão mantém o que já está no banco
    if (meta.nome)              updatePayload.nome              = meta.nome;
    if (meta.crm)               updatePayload.crm               = meta.crm;
    if (meta.especialidade)     updatePayload.especialidade     = meta.especialidade;
    if (meta.email)             updatePayload.email             = meta.email;
    if (meta.celular)           updatePayload.celular           = meta.celular;
    if (meta.endereco)          updatePayload.endereco          = meta.endereco;
    if (meta.cidade)            updatePayload.cidade            = meta.cidade;
    if (meta.uf)                updatePayload.uf                = meta.uf;
    if (meta.scrub)             updatePayload.scrub             = meta.scrub;
    if (meta.restricao)         updatePayload.restricao_alimentar = meta.restricao;
    if (meta.restricao_detalhe) updatePayload.restricao_detalhe = meta.restricao_detalhe;

    // Tenta PATCH no registro existente pelo external_reference
    const patchRes = await fetch(
      `${SUPABASE_URL}/rest/v1/inscricoes_osteotomia?mp_external_reference=eq.${externalRef}`,
      {
        method:  'PATCH',
    ...
    if (patchRes.ok) {
      ...
      if (updated && updated.length > 0) {
        inscricao = updated[0];
        console.log('Inscrição atualizada via PATCH:', externalRef, payment.status);
      }
    }

    // Fallback: se não achou pelo external_reference, tenta pelo mp_payment_id
    if (!inscricao) {
      const patchById = await fetch(
        `${SUPABASE_URL}/rest/v1/inscricoes_osteotomia?mp_payment_id=eq.${payment.id}`,
        {
          method:  'PATCH',
          headers: { ...sbHeaders, 'Prefer': 'return=representation' },
          body:    JSON.stringify(updatePayload)
      ...
      if (patchById.ok) {
        const updated = await patchById.json();
        if (updated && updated.length > 0) {
          inscricao = updated[0];
          console.log('Inscrição atualizada via PATCH by payment_id:', payment.id);
        }
      }
    }

    // Último recurso: INSERT com todos os dados disponíveis
    if (!inscricao) {
      console.warn('Registro não encontrado, fazendo INSERT:', externalRef);
      const insertPayload = {
        ...updatePayload,
      ...
      const insRes = await fetch(`${SUPABASE_URL}/rest/v1/inscricoes_osteotomia`, {
        method:  'POST',
        headers: { ...sbHeaders, 'Prefer': 'return=representation' },
      ...
      if (insRes.ok) {
        ...
        inscricao = inserted?.[0] || insertPayload;
        console.log('Inscrição inserida via INSERT:', externalRef);
      } else {
    ...
    if (payment.status === 'approved' && totalTaxas > 0) {
      const nomeRef = inscricao?.nome ? `Inscrição ${inscricao.nome}` : `Pagamento ${paymentId}`;
      const lancamentos = [];
      ...
      if (taxaParc > 0) {
        lancamentos.push({
          tipo:            'despesa',
          categoria:       'Taxas MP',
          descricao:       `Taxa parcelamento — ${nomeRef}`,
      ...
      for (const lanc of lancamentos) {
        ...
        if (!fRes.ok) console.error('Supabase fluxo error:', await fRes.text());
      }

      console.log(`Taxas registradas: MP R$${taxaMP.toFixed(2)}, Parc R$${taxaParc.toFixed(2)}`);
    }

    // ── 5. E-mail de confirmação (só aprovados, só uma vez) ───────────────
    if (payment.status === 'approved' && RESEND_API_KEY) {
      const emailDest = inscricao?.email || meta.email || payment.payer?.email;
      const nomeDoc   = inscricao?.nome  || meta.nome  || 'Médico(a)';

      // Só envia se tiver e-mail real e não for dados pendentes
      if (emailDest && emailDest !== '—' && !inscricao?.notificacao_enviada) {
